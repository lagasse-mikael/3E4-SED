...
class ElementsRoutes {
    ...
    delete(req, res, next) {
        const verifElementDelete = ELEMENTS.find(element => element.symbol == req.params.symbol)
        if(verifElementDelete != undefined)
        {
            ELEMENTS.delete(verifElementDelete);
            res.status(httpStatus.OK)
        }
        else
            return next(HttpErrors.NotFound("L'element n'existe pas"))
    }
}
